...
% prepare
Wgen     = [re im]; % Nx2, you may extend to NxT
...
tic;
Result2 = BSBL_FM(Phi,Y,blkStartLoc,learnLambda,'learnType',0,'verbose',0);
t_fm2 = toc;
...
clf;
subplot(121)
plot(Wgen(:,1),'b-','linewidth',2); hold on
plot(Wgen(:,2),'r--','linewidth',2); grid on; axis tight
hx1 = xlabel('(a) Original 2-channel Signal'); hy1 = ylabel('Amplitude');
ax1 = gca;
hl = legend('Channel -- 1','Channel -- 2');
subplot(122)
plot(Result2.x(:,1),'b-','linewidth',2); hold on
plot(Result2.x(:,2),'r--','linewidth',2); grid on; axis tight
